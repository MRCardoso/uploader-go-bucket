...
| ---------------------------------------------------------------------------------------
*/
function s3Helper(options = {bucket:'', timeout: 0})
{
...
function _manageObject(path, image, postImage, sessionImage)
{
    var $this = this;
    var promises = [];
    var removeImageS3 = function(Keys)
    {
        return new Promise((resolve, reject)=> {
            $this
            ._deleteObject(Keys)
            .then(data =>{
                ...
                reject({ errors: [{ message: err.message }] });
            });
        });
    };
    ...
    return new Promise( (resolve, reject) =>
    {
        if( image != null )
        {
            $this._listObject(path).then(response =>
            {
                var Keys = [];
                response
                ...
                if( Keys.length > 0 ) {
                    promises.push(removeImageS3(Keys));
                }
            }, err => {
        ...
        if( (postImage == null && sessionImage == null) && image != null){
            promises.push(removeImageS3([{Key: `${path}/${image.path}`}]));
        }
